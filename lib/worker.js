...
Worker.prototype.runServer = function (config) {
    ...
    var startHandler = function (req, res) {
        ...
        if (config.newRelic) {
            req.headers['X-Request-Start'] = (new Date().getTime()) * 1000;
        }
    };
    ...
    var httpRequestHandler = function (req, res) {
        ...
        (function () {
            ...
            res.end = function () {
                ...
                self.stats.countTransaction(req, res);
                // Log the request
                log({
